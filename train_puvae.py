...
class PuVAE(nn.Module):
    def __init__(self, imageShape, labelSize):
        super(PuVAE, self).__init__()
 
        # Encoder
        self.conv0 = nn.Conv2d(imageShape[0] + labelSize, 64, kernel_size=5, stride=1, padding=2, bias= False)
        ...
        # Decoder
        self.fc3 = nn.Linear(128 + labelSize, 4096)
        ...
        self.deconv4 = nn.ConvTranspose2d(64, imageShape[0], kernel_size=5, stride=1, padding=2, bias=False)
        self.relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()
 
    def encode(self, x, y):
        l = torch.reshape(y, (len(y), y.shape[1], 1, 1))
        k = torch.ones(len(y), y.shape[1], x.shape[2], x.shape[2])
        k = k * l
...
def train(epoch, classifier):
    ...
    for batch_idx, (adv_batch, clean_batch, y_batch) in enumerate(train_data_loader):
        ...
        optimizer.zero_grad()
        recon_batch, mu, logvar = model(adv_batch, y_batch)
...
model = PuVAE(x_train.shape[1:], y_train.shape[1]).to(device)
